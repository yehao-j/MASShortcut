...
- (void)setShortcutValue:(MASShortcut *)shortcutValue {
    ...
    if (shortcutValue == nil) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ChangeShortCut" object:@{@"key": self.associatedUserDefaultsKey}];
    } else {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ChangeShortCut" object:@{@"key": self.associatedUserDefaultsKey, @"value": shortcutValue}];
    }
...
- (void)setAssociatedUserDefaultsKey:(NSString *)associatedUserDefaultsKey {
    _associatedUserDefaultsKey = associatedUserDefaultsKey;
    
    NSData *data = [[NSUserDefaults standardUserDefaults] valueForKey:associatedUserDefaultsKey];
    if (data) {
        MASShortcut *archiveShortcut = [NSKeyedUnarchiver unarchivedObjectOfClass:[MASShortcut class] fromData:data error:nil];
        if (archiveShortcut) {
            self.shortcutValue = archiveShortcut;
        }
    }
}
